...
def main(config):
    ...
    generator = UnetGeneratorMultiHead(input_nc=3, output_nc=50, num_heads=3, num_downs=5, ngf=64)
    generator.to(device)
    print(generator)
    ...
    for epoch in range(config.max_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            x_logits, y_logits, z_logits, nocs_estimated, masks_estimated = generator(rgb_images_gt)
            ...
            if config.with_transformer_loss:
                regression_nocs_loss = transformer_loss(nocs_estimated, nocs_images_normalized_gt, transformations)
            else:
            ...
            background_region_gt = (nocs_images_normalized_gt == 0).all(dim=1).float()
            background_region_pred = (nocs_estimated == 0).all(dim=1).float()

            # Calculate the loss between the two background masks
            bg_loss = F.mse_loss(background_region_pred, background_region_gt) * 5
