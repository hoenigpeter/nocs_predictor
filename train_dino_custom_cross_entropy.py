...
from model import AutoencoderXYZHead as ae
...
def main():
    ...
    weight_dir = "./weights_custom_cross_entropy"
    if not(os.path.exists(weight_dir)):
            os.makedirs(weight_dir)

    val_img_dir = "./val_img_custom_cross_entropy"
    ...
    optimizer_generator = optim.Adam(generator.parameters(), weight_decay=0.00004, lr=0.00003)
    ...
    for epoch in range(max_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            x_channel_gt = torch.zeros(batch_size, 256, 256, 256, device=device, dtype=torch.float32)
            x_channel_gt.scatter_(1, nocs_images_split[:,0].unsqueeze(1), 1.0)

            y_channel_gt = torch.zeros(batch_size, 256, 256, 256, device=device)
            y_channel_gt.scatter_(1, nocs_images_split[:,1].unsqueeze(1), 1.0)

            z_channel_gt = torch.zeros(batch_size, 256, 256, 256, device=device)
            ...
            #loss = (r_loss.sum() + g_loss.sum() + b_loss.sum())
