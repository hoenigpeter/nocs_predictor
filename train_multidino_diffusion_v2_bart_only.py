...
from diffusion_model import DiffusionNOCSBART
...
def main(config):
    ...
    generator = DiffusionNOCSBART(input_nc = 9, output_nc = 3, image_size=config.image_size, num_training_steps=config.num_training_steps, num_inference_steps=config.num_inference_steps)
    generator.to(device)
    print(generator)
    ...
    for epoch in range(config.max_epochs):
        ...
        bart_embeddings = generator.get_bart_embeddings(obj_names)
        nocs_estimated = generator.inference(rgb_images_gt, normal_images_gt, bart_embeddings)
